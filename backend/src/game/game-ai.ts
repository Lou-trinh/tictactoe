...
const moveHeuristic = (
  ...
  scoreMoveForPlayer(board, move, player) * 1.08 +
  scoreMoveForPlayer(board, move, opponent);
...
const orderedCandidates = (
  ...
  moves.sort(
    (left, right) => right.score - left.score || compareMoves(left, right),
  );
  return typeof limit === 'number' ? moves.slice(0, limit) : moves;
};

const bestPositionPotential = (
  board: SparseBoard,
  player: PlayerSymbol,
  opponent: PlayerSymbol,
...
const minimax = (
  ...
  if (
    lastMove &&
    findWinningLine(board, lastMove.row, lastMove.col, lastMove.player)
  ) {
...
export const chooseAiMove = (
  ...
  if (difficulty === 'normal') {
    const variedChoices = orderedMoves.slice(
      0,
      Math.min(3, orderedMoves.length),
    );
    const selectedMove =
      variedChoices[Math.floor(random() * variedChoices.length)];
    return selectedMove
      ? { row: selectedMove.row, col: selectedMove.col }
      : { row: 0, col: 0 };
  }
